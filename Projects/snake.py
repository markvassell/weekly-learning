...
class Player:
    def __init__(self):
        ...
        self.height = BLOCK_SIZE
        self.width = BLOCK_SIZE
        self.snake = pygame.Rect(randint(5,595), randint(5,595), self.height, self.width)
        self.score = 0
        self.direction = R
    ...
    def check_position(self):
        if self.snake.bottom > 600:
            self.snake.top = 0
        elif self.snake.top < 0:
            self.snake.bottom = 600
        elif self.snake.left < 0:
            self.snake.right = 600
        elif self.snake.right > 600:
            self.snake.left = 0
...
class Game:
    ...
    def check_for_eat_event(self):
        if self.snake.snake.colliderect(self.food.food):
            self.food = Food()
